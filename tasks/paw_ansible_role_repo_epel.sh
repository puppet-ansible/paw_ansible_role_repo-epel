...
# Puppet task for executing Ansible role: ansible_role_repo_epel
# This script runs the entire role via ansible-playbook

# Determine the ansible modules directory
if [ -n "$PT__installdir" ]; then
  ANSIBLE_DIR="$PT__installdir/lib/puppet_x/ansible_modules/ansible_role_repo_epel"
else
  # Fallback to /opt/puppetlabs/puppet/cache/lib/puppet_x/ansible_modules
  ANSIBLE_DIR="/opt/puppetlabs/puppet/cache/lib/puppet_x/ansible_modules/ansible_role_repo_epel"
fi
...
if [ -d "$ANSIBLE_DIR/roles" ] && [ -f "$ANSIBLE_DIR/roles/paw_ansible_role_repo_epel/playbook.yml" ]; then
  # Collection structure
  PLAYBOOK_PATH="$ANSIBLE_DIR/roles/paw_ansible_role_repo_epel/playbook.yml"
  PLAYBOOK_DIR="$ANSIBLE_DIR/roles/paw_ansible_role_repo_epel"
  ...
  echo "{\"_error\": {\"msg\": \"playbook.yml not found in $ANSIBLE_DIR or $ANSIBLE_DIR/roles/paw_ansible_role_repo_epel\", \"kind\": \"puppet-ansible-converter/playbook-not-found\"}}"
  exit 1
fi
...
EXTRA_VARS="{"
FIRST=true
if [ -n "$PT_epel_repo_url" ]; then
  if [ "$FIRST" = true ]; then
    FIRST=false
  else
    EXTRA_VARS="$EXTRA_VARS,"
  fi
  EXTRA_VARS="$EXTRA_VARS\"epel_repo_url\": \"$PT_epel_repo_url\""
fi
if [ -n "$PT_epel_repo_gpg_key_url" ]; then
  if [ "$FIRST" = true ]; then
    FIRST=false
  else
    EXTRA_VARS="$EXTRA_VARS,"
  fi
  EXTRA_VARS="$EXTRA_VARS\"epel_repo_gpg_key_url\": \"$PT_epel_repo_gpg_key_url\""
fi
if [ -n "$PT_epel_repofile_path" ]; then
  if [ "$FIRST" = true ]; then
    FIRST=false
  else
    EXTRA_VARS="$EXTRA_VARS,"
  fi
  EXTRA_VARS="$EXTRA_VARS\"epel_repofile_path\": \"$PT_epel_repofile_path\""
fi
if [ -n "$PT_epel_repo_disable" ]; then
  if [ "$FIRST" = true ]; then
    FIRST=false
  else
    EXTRA_VARS="$EXTRA_VARS,"
  fi
  EXTRA_VARS="$EXTRA_VARS\"epel_repo_disable\": \"$PT_epel_repo_disable\""
fi
EXTRA_VARS="$EXTRA_VARS}"
...
if [ $EXIT_CODE -eq 0 ]; then
  echo '{"status": "success", "role": "ansible_role_repo_epel"}'
else
  echo "{\"status\": \"failed\", \"role\": \"ansible_role_repo_epel\", \"exit_code\": $EXIT_CODE}"
fi
